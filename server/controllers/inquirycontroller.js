...
exports.createInquiry = async (req, res) => {
    try {
        const { propertyId, message, inquiryType, preferredVisitDate, preferredVisitTime, phone, email } = req.body;
...
exports.respondToInquiry = async (req, res) => {
    try {
        ...
        if (inquiry.receiver.toString() === req.user.id && inquiry.status === "pending") {
...
exports.deleteInquiry = async (req, res) => {
    try {
        ...
        if (inquiry.sender.toString() !== req.user.id && req.user.role !== "admin") {
            return res.status(403).json({
                success: false,
...
exports.getInquiryStats = async (req, res) => {
    try {
        ...
        const totalReceived = await Inquiry.countDocuments({ receiver: req.user.id });
        const totalSent = await Inquiry.countDocuments({ sender: req.user.id });
        const unread = await Inquiry.countDocuments({ receiver: req.user.id, isRead: false });
